...
namespace NUnit {
	...
	public static class ResultAssert {
		...
		public static void Fails(Result actual) {
			if (actual) {
				throw new AssertionException($"ResultAssert.Fails failed. Parser succeeded when expected to fail");
			}
		}
		...
		public static void Captures(String expected, Result actual) {
			...
			if (expected != actual) {
				throw new AssertionException($"ResultAssert.Captures failed. Expected: <{expected}>. Actual: <{(String)actual}>.");
			}
		}
