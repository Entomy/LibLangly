...
namespace System.Traits.Providers {
	public static partial class Collection {
		...
		public static Boolean ContainsAny<TElement>(TElement[] collection, Int32 count, ReadOnlySpan<TElement> elements) {
		...
		/// <returns><see langword="true"/> if any of the <paramref name="elements"/> are contained in this collection; otherwise <see langword="false"/>.</returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Boolean ContainsAny<TNode, TElement>(TNode head, ReadOnlySpan<TElement> elements) where TNode : class, IContains<TElement>, INext<TNode?> {
			Boolean[] found = new Boolean[elements.Length];
			TNode? N = head;
			while (N is not null) {
