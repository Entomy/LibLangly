...
namespace Langly {
	public class Glyph_Tests {
		...
		public void Compare(String first, String second, Int32 expected) {
			Glyph a = Glyph.Parse(first);
			Glyph b = Glyph.Parse(second);
		...
		[InlineData("\u0041\u030A")] // Å
		[InlineData("\uD834\uDD1E")] // 𝄞
		public void Parse(String sequence) => Glyph.Parse(sequence);
		...
		public void EncodeToUtf16(String sequence, Int32 length) {
			Glyph glyph = Glyph.Parse(sequence);
		...
		public void Equals_Glyph(String first, String second) {
			Assert.Equal(Glyph.Parse(first), Glyph.Parse(second));
			Assert.Equal(Glyph.Parse(second), Glyph.Parse(first));
		}
		...
		public void GetGlyphAt(String input, Int32 index, String seq, Int32 nxt) {
			...
			Glyph expected = Glyph.Parse(seq);
		...
		public void Reverse(String source, String expected) => Assert.True(String.Equals(expected, Glyph.Reverse(source).ToString(), StringComparison.InvariantCulture));
		...
		//[InlineData("\u0049", "\u0131")] // I >=> ı
		//public void ToLower_azAZ(String sequence, String expected) => Assert.Equal(Glyph.Parse(expected), Glyph.Parse(sequence).ToLower(Azerbaijani[Latin]));
		...
		[InlineData("\u0041\u0300", "\u0061\u0300")] // À >=> à
		[InlineData("\u0049", "\u0069")] // I >=> i
		public void ToLower_Invariant(String sequence, String expected) => Assert.Equal(Glyph.Parse(expected), Glyph.Parse(sequence).ToLower());
		...
		//[InlineData("\u0041\u0300", "\u0061\u0300")] // À >=> à
		//[InlineData("\u0049", "\u0131")] // I >=> ı
		//public void ToLower_trTR(String sequence, String expected) => Assert.Equal(Glyph.Parse(expected), Glyph.Parse(sequence).ToLower(Turkish[Latin]));
		...
		//[InlineData("\u0061\u0300", "\u0041\u0300")] // à >=> À
		//[InlineData("\u0069", "\u0130")] // i >=> İ
		//public void ToUpper_azAZ(String sequence, String expected) => Assert.Equal(Glyph.Parse(expected), Glyph.Parse(sequence).ToUpper(Azerbaijani[Latin]));
		...
		public void ToUpper_Invariant(String sequence, String expected) {
			Glyph exp = Glyph.Parse(expected);
			Glyph seq = Glyph.Parse(sequence);
		...
		//[InlineData("\u0061\u0300", "\u0041\u0300")] // à >=> À
		//[InlineData("\u0069", "\u0130")] // i >=> İ
		//public void ToUpper_trTR(String sequence, String expected) => Assert.Equal(Glyph.Parse(expected, Glyph.Parse(sequence).ToUpper(Turkish[Latin]));
	}
}
