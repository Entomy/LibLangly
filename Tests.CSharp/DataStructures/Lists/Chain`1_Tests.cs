using System;
using Xunit;

namespace Langly.DataStructures.Lists {
	public class Chain1_Tests {
		[Fact]
		public void Constructor() {
			Chain<Int32> chain = new Chain<Int32>();
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Single), MemberType = typeof(Collection1_Data))]
		public void Add_Element(Int32[] expected) {
			Chain<Int32> chain = new Chain<Int32>();
			foreach (Int32 item in expected) {
				chain = chain.Add(item);
			}
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Group), MemberType = typeof(Collection1_Data))]
		public void Add_Array(Int32[] expected, Int32[] slicePoints) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				chain = chain.Add(expected[i..sp]);
				i = sp;
			}
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Group), MemberType = typeof(Collection1_Data))]
		public void Add_Memory(Int32[] expected, Int32[] slicePoints) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(expected[i..sp].AsMemory());
				i = sp;
			}
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Group), MemberType = typeof(Collection1_Data))]
		public void Add_ReadOnlyMemory(Int32[] expected, Int32[] slicePoints) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add((ReadOnlyMemory<Int32>)expected[i..sp].AsMemory());
				i = sp;
			}
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Group), MemberType = typeof(Collection1_Data))]
		public void Add_Span(Int32[] expected, Int32[] slicePoints) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(expected.AsSpan()[i..sp]);
				i = sp;
			}
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Group), MemberType = typeof(Collection1_Data))]
		public void Add_ReadOnlySpan(Int32[] expected, Int32[] slicePoints) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add((ReadOnlySpan<Int32>)expected[i..sp].AsSpan());
				i = sp;
			}
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Single), MemberType = typeof(Collection1_Data))]
		public void Indexer_Element(Int32[] expected) {
			Chain<Int32> chain = new Chain<Int32>();
			foreach (Int32 item in expected) {
				_ = chain.Add(item);
			}
			for (nint i = 0; i < expected.Length; i++) {
				Assert.Equal(expected[i], chain[i]);
			}
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Add_Int32_Group), MemberType = typeof(Collection1_Data))]
		public void Indexer_Memory(Int32[] expected, Int32[] slicePoints) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(expected[i..sp]);
				i = sp;
			}
			for (i = 0; i < expected.Length; i++) {
				Assert.Equal(expected[i], chain[i]);
			}
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Single_Single), MemberType = typeof(Collection1_Data))]
		public void Insert_Element_Element(Int32[] expected, Int32[] initial, Int32 index, Int32 element) {
			Chain<Int32> chain = new Chain<Int32>();
			foreach (Int32 item in initial) {
				_ = chain.Add(item);
			}
			_ = chain.Insert(index, element);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Single_Group), MemberType = typeof(Collection1_Data))]
		public void Insert_Element_Memory(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 index, Int32 element) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial[i..sp]);
				i = sp;
			}
			_ = chain.Insert(index, element);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Single_Group), MemberType = typeof(Collection1_Data))]
		public void Insert_Element_Span(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 index, Int32 element) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial.AsSpan()[i..sp]);
				i = sp;
			}
			_ = chain.Insert(index, element);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Group_Single), MemberType = typeof(Collection1_Data))]
		public void Insert_Memory_Element(Int32[] expected, Int32[] initial, Int32 index, Int32[] elements) {
			Chain<Int32> chain = new Chain<Int32>();
			foreach (Int32 item in initial) {
				_ = chain.Add(item);
			}
			_ = chain.Insert(index, elements);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Group_Single), MemberType = typeof(Collection1_Data))]
		public void Insert_Span_Element(Int32[] expected, Int32[] initial, Int32 index, Int32[] elements) {
			Chain<Int32> chain = new Chain<Int32>();
			foreach (Int32 item in initial) {
				_ = chain.Add(item);
			}
			_ = chain.Insert(index, elements.AsSpan());
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Group_Group), MemberType = typeof(Collection1_Data))]
		public void Insert_Memory_Memory(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 index, Int32[] elements) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial[i..sp]);
				i = sp;
			}
			_ = chain.Insert(index, elements);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Insert_Int32_Group_Group), MemberType = typeof(Collection1_Data))]
		public void Insert_Span_Span(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 index, Int32[] elements) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial.AsSpan()[i..sp]);
				i = sp;
			}
			_ = chain.Insert(index, elements.AsSpan());
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Replace_Single), MemberType = typeof(Collection1_Data))]
		public void Replace_Element(Int32[] expected, Int32[] initial, Int32 search, Int32 replace) {
			Chain<Int32> chain = new Chain<Int32>();
			foreach (Int32 item in initial) {
				_ = chain.Add(item);
			}
			_ = chain.Replace(search, replace);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Replace_Group), MemberType = typeof(Collection1_Data))]
		public void Replace_Array(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 search, Int32 replace) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial[i..sp]);
				i = sp;
			}
			_ = chain.Replace(search, replace);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Replace_Group), MemberType = typeof(Collection1_Data))]
		public void Replace_Memory(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 search, Int32 replace) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial[i..sp].AsMemory());
				i = sp;
			}
			_ = chain.Replace(search, replace);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Replace_Group), MemberType = typeof(Collection1_Data))]
		public void Replace_ReadOnlyMemory(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 search, Int32 replace) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add((ReadOnlyMemory<Int32>)initial[i..sp].AsMemory());
				i = sp;
			}
			_ = chain.Replace(search, replace);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Replace_Group), MemberType = typeof(Collection1_Data))]
		public void Replace_Span(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 search, Int32 replace) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add(initial.AsSpan()[i..sp]);
				i = sp;
			}
			_ = chain.Replace(search, replace);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Replace_Group), MemberType = typeof(Collection1_Data))]
		public void Replace_ReadOnlySpan(Int32[] expected, Int32[] initial, Int32[] slicePoints, Int32 search, Int32 replace) {
			Chain<Int32> chain = new Chain<Int32>();
			Int32 i = 0;
			foreach (Int32 sp in slicePoints) {
				_ = chain.Add((ReadOnlySpan<Int32>)initial.AsSpan()[i..sp]);
				i = sp;
			}
			_ = chain.Replace(search, replace);
			Assert.Equal(expected, chain);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.Sparse), MemberType = typeof(Collection1_Data))]
		public void Sparse(Int32 expected, Int32[] values, Int32 index) {
			Chain<Int32> chain = new Chain<Int32>(Filter.Sparse).Add(values);
			Assert.Equal(expected, chain[index]);
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.ToString), MemberType = typeof(Collection1_Data))]
		public void ToString(String expected, Int32[] values) {
			Chain<Int32> chain = new Chain<Int32>().Add(values);
			Assert.Equal(expected, chain.ToString());
		}

		[Theory]
		[MemberData(nameof(Collection1_Data.ToString_Amount), MemberType = typeof(Collection1_Data))]
		public void ToString_Amount(String expected, Int32[] values, Int32 amount) {
			Chain<Int32> chain = new Chain<Int32>().Add(values);
			Assert.Equal(expected, chain.ToString(amount));
		}
	}
}
