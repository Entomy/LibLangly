...
namespace Langly.DataStructures {
	/// <summary>
	/// Represents the disjunction between two <see cref="Set{TElement}"/>.
	/// </summary>
	...
	internal sealed class Disjunction<TElement, TFirst, TSecond> : Combinator<TElement, TFirst, TSecond>
		where TFirst : Set<TElement>
		where TSecond : Set<TElement> {
		/// <summary>
		/// Initializes a new <see cref="Disjunction{TElement, TFirst, TSecond}"/>.
		/// </summary>
		/// <param name="first">The first set.</param>
		/// <param name="second">The second set.</param>
		internal Disjunction([DisallowNull] TFirst first, [DisallowNull] TSecond second) : base(first, second) { }
