...
namespace Langly.DataStructures.Lists {
	public partial class Chain<TIndex, TElement> : ISequenceBidi<(TIndex Index, TElement Element), Chain<TIndex, TElement>.Enumerator> {
		/// <inheritdoc/>
		...
		public struct Enumerator : IEnumeratorBidi<(TIndex Index, TElement Element)> {
			[DisallowNull]
			private readonly Chain<TIndex, TElement> Chain;
			...
			/// </summary>
			/// <param name="chain">The <see cref="Chain{TIndex, TElement}"/> to enumerate.</param>
			public Enumerator([DisallowNull] Chain<TIndex, TElement> chain) {
				Chain = chain;
				N = null;
				i = -1;
			}
			...
			/// <inheritdoc/>
			public (TIndex Index, TElement Element) Current => N[i];
