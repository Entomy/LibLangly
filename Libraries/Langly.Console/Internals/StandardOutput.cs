...
namespace Langly.Internals {
	...
	internal sealed class StandardOutput : IConsoleWriter {
		...
		/// <inheritdoc/>
		[return: NotNull]
		unsafe IConsoleWriter IAdd<Char, IConsoleWriter>.Add(Char element) {
#if WINDOWS
		...
		/// <inheritdoc/>
		[return: NotNull]
		unsafe IConsoleWriter IAddUnsafe<Char, IConsoleWriter>.Add([AllowNull] Char* elements, Int32 length) {
			if (elements is null) {
				return this;
			}
#if WINDOWS
