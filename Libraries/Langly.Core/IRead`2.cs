...
namespace Langly {
	...
	public interface IRead<TElement, TError> {
		/// <summary>
	...
	public static partial class TraitExtensions {
		...
		/// <remarks>
		/// This assumes the read will never fail, which is an assumption that generally doesn't hold up. It is strongly recommended to use <see cref="TryRead{TElement, TError}(IRead{TElement, TError}, out TElement)"/> or <see cref="TryRead{TElement, TError}(IRead{TElement, TError}, out TElement, out TError)"/> instead.
		/// </remarks>
		public static void Read<TElement, TError>([AllowNull] this IRead<TElement, TError> collection, [MaybeNull] out TElement element) {
		...
		/// <param name="element">The element that was read.</param>
		/// <returns><see langword="true"/> if the read was successful; otherwise, <see langword="false"/>.</returns>
		public static Boolean TryRead<TElement, TError>([AllowNull] this IRead<TElement, TError> collection, [MaybeNullWhen(false), NotNullWhen(true)] out TElement element) => TryRead(collection, out element, out _);
		...
		/// <param name="error">The <typeparamref name="TError"/> that occurred, if any.</param>
		/// <returns><see langword="true"/> if the read was successful; otherwise, <see langword="false"/>.</returns>
		public static Boolean TryRead<TElement, TError>([AllowNull] this IRead<TElement, TError> collection, [MaybeNullWhen(false), NotNullWhen(true)] out TElement element, [MaybeNullWhen(true), NotNullWhen(false)] out TError error) {
			if (collection is null) {
				element = default;
				error = default;
				return true;
