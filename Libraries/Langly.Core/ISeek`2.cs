...
namespace Langly {
	...
	public interface ISeek<TElement, TError> {
		/// <summary>
		/// Can this be seeked?
		/// </summary>
		/// <remarks>
	...
	public static partial class TraitExtensions {
		...
		/// <param name="offset">The offset of <typeparamref name="TElement"/> from the current position to seek to.</param>
		public static void Seek<TElement, TError>([MaybeNull] this ISeek<TElement, TError> collection, nint offset) => collection?.Seek(offset);
		...
		/// <param name="offset">The offset of <typeparamref name="TElement"/> from the current position to seek to.</param>
		/// <returns><see langword="true"/> if the seek was successful; otherwise, <see langword="false"/>.</returns>
		/// <seealso cref="TrySeek{TElement, TError}(ISeek{TElement, TError}, nint, out TError)"/>
		public static Boolean TrySeek<TElement, TError>([MaybeNull] this ISeek<TElement, TError> collection, nint offset) => collection?.TrySeek(offset) ?? false;
		...
		/// <param name="error">The <typeparamref name="TError"/> that occurred, if any.</param>
		/// <returns><see langword="true"/> if the seek was successful; otherwise, <see langword="false"/>.</returns>
		/// <seealso cref="TrySeek{TElement, TError}(ISeek{TElement, TError}, nint)"/>
		public static Boolean TrySeek<TElement, TError>([MaybeNull] this ISeek<TElement, TError> collection, nint offset, [MaybeNullWhen(true)] out TError error) {
