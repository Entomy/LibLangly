    ...
    internal sealed class UnicodeDataFileEntry
    {
        ...
        public UnicodeDataFileEntry(string line)
        {
            ...
            if (!string.IsNullOrEmpty(split[8]))
            {
                ...
                {
                    double numerator = double.Parse(numericValue.AsSpan(0, indexOfSlash).ToString(), NumberStyles.Integer, CultureInfo.InvariantCulture);
                    double denominator = double.Parse(numericValue.AsSpan(indexOfSlash + 1).ToString(), NumberStyles.Integer, CultureInfo.InvariantCulture);
