...
namespace Langly {
	...
	public interface IRefIndexable<TElement> : IRefIndexable<nint, TElement>, IReadOnlyRefIndexable<TElement> {
		/// <summary>
	...
	public static partial class TraitExtensions {
		...
		/// <param name="func">The <see cref="Func{T, TResult}"/> to apply to each element.</param>
		/// <remarks>
		/// This is a mutating operation. It mutates <paramref name="collection"/> rather than constructing a new collection for the result. This is done for two reasons. First is that we can't generalize <see cref="Map{TElement}(IRefIndexable{TElement}, Func{TElement, TElement})"/> like this while also allowing arbitrary collections to be constructed. Second is performance, as mutating in-place is substantially more efficient. <see cref="Philosoft"/> is not a pure functional programming library, it just takes inspiration in the form of more practical application of many functional principals.
		/// </remarks>
		public static void Map<TElement>([AllowNull] this IRefIndexable<TElement> collection, [AllowNull] Func<TElement, TElement> func) {
