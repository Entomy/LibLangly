...
namespace Langly {
	...
	public interface IReadOnlyRefIndexable<TElement> : IReadOnlyRefIndexable<nint, TElement> {
	}
	...
	public static partial class TraitExtensions {
		...
		public static nint IndexOf<TElement>(this IReadOnlyRefIndexable<TElement> collection, [MaybeNull] Object item) {
		...
		/// <param name="item">The item to locate in the collection.</param>
		/// <returns>The index of the first occurrence of <paramref name="item"/> in <paramref name="collection"/>, if found; otherwise, <c>-1</c>.</returns>
		public static nint IndexOf<TElement>(this IReadOnlyRefIndexable<TElement> collection, [MaybeNull] TElement item) where TElement : IEquatable<TElement> {
			if (collection is null) {
				return -1;
			}
			for (nint i = 0; i < collection.Count; i++) {
		...
		public static nint IndexOf<TElement>(this IReadOnlyRefIndexable<TElement> collection, [MaybeNull] Predicate<TElement> match) {
			if (collection is null) {
				return -1;
			}
			if (match is null) {
