...
// This file has been adapted for use with MSTest instead of xunit. It is otherwise identical to the official one.
...
namespace Tests {
	...
	public static class UnicodeData {
		...
		static UnicodeData() {
			static IEnumerable<String> ReadAllLines(String resourceName) {
				using (Stream stream = new FileStream(resourceName, FileMode.Open)) {
					if (stream is null) {
						throw new IOException($"Could not locate resource stream {resourceName} in the assembly.");
					}
			...
			foreach (string line in ReadAllLines(UnicodeDataFilename)) {
				...
				Assert.AreEqual(15, splitLine.Length); // expected 15 segments resulting from the split
				...
				if (codePointName.EndsWith(", First>", StringComparison.Ordinal)) {
					...
					nameOfCurrentRange = codePointName.Substring(0, codePointName.Length - ", First>".Length);
					continue;
				}
			...
			foreach (string line in ReadAllLines(PropListFilename)) {
				...
				CharProperty property = (CharProperty)Enum.Parse(typeof(CharProperty), match.Groups["propName"].Value);
		...
		public static UnicodeCategory GetUnicodeCategory(uint codePoint) {
			Assert.IsTrue(codePoint <= 0x10FFFF, "Invalid code point provided.");
